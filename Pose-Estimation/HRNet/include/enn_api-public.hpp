 ...
 * @param buffers_info  number of in / out buffer.
 ...
 *
 * @param model_id model ID from load_model
 * @param batch_info batch buffer info (EnnBatchBufferInfo) filled by user.
 * @param num_batch num of inference in a batch
 * @param is_chunk bool flag to inform each ifm/ofm is chunk buffer for all batch inferences
 * @return EnnReturn zero if successful
 */
...
/**
 * @brief Unset multiple buffers to buffer space once.
 * @param model_id [IN] model ID from load_model
 * @param session_id [IN] If a caller generates 2 or more buffer space, session_id can be an identifier
 * @ingroup api_model
 * @return EnnReturn result, 0 is success
...
/**
 * @brief Cancle buffer commit. Uncommited buffer could be reused by calling BufferCommit() again.
 * Uncommit is optional for reusing buffer. Normally, calling CloseModel() is enough to release commited buffers.
 * @param model_id [IN] model ID from load_model
 * @ingroup api_commit
 * @return EnnReturn result, 0 is success
 */
