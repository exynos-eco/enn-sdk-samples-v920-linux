...
int run_damo_yolo_detection(const std::string& model_file, const std::string& input_file) {
    ...
    if (!enn.initialize(model_file)) {
        PRINT_ERROR("Failed to initialize ENN model.");
        return 0;
    }
    ...
    if (input_type == 1) {
        ...
        enn.run(img_frame, detect_list);
        cv::Mat vis = enn::sample_utils::DetectListDrawer::draw(img_frame, detect_list);
        last_frame = vis.clone();

        gst.push_frame(vis, frame_count++);
        cv::imwrite("/tmp/damo_yolo_result.jpg", vis);     
    } else {
        while (keep_running) {
            cv::Mat frame;
            ...
            std::vector<DL_RESULT> detect_list;
            enn.run(frame, detect_list);
            cv::Mat vis = enn::sample_utils::DetectListDrawer::draw(frame, detect_list);
